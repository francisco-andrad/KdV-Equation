...
void space_finite_diff(double *x, double *aux, double dx)
{
    ...
    // derivada terceira:
    aux[i] += -((((x[3] + x[2]) / 2.0) - (x[2] + x[1]) + (0.0) - (0.0)) / (2.0 * dx * dx * dx));
    ...
    aux[i] = (2.0 * mu * (std::abs(x[i + 1]) + std::abs(x[i]) + std::abs(x[i - 1])) / 3.0) *
             ((x[i + 1] - x[i - 1]) / (2.0 * dx));
    // derivada terceira:
    aux[i] += -((((x[4] + x[3]) / 2.0) - (x[3] + x[2]) + (x[0] + 0.0) - (0.0))) / (2.0 * dx * dx * dx);
    ...
    for (i = 2; i < space_steps - 2; i++)
    {
        ...
        // derivada terceira:
        aux[i] += -((((x[i + 3] + x[i + 2]) / 2.0) - (x[i + 2] + x[i + 1]) + (x[i] + x[i - 1]) -
                     ((x[i - 1] + x[i - 2]) / 2.0))) /
                  (2.0 * dx * dx * dx);
    }
    ...
    aux[i] = (2.0 * mu * (std::abs(x[i + 1]) + std::abs(x[i]) + std::abs(x[i - 1])) / 3.0) *
             ((x[i + 1] - x[i - 1]) / (2.0 * dx));
    // derivada terceira:
    aux[i] += -(((0.0) - (0.0 + x[space_steps - 1]) + (x[space_steps - 2] + x[space_steps - 3]) -
                 ((x[space_steps - 3] + x[space_steps - 4]) / 2.0))) /
              (2.0 * dx * dx * dx);
    ...
    // derivada terceira:
    aux[i] += -(((0.0) - (0.0) + (x[space_steps - 1] + x[space_steps - 2]) -
                 ((x[space_steps - 2] + x[space_steps - 3]) / 2.0))) /
              (2.0 * dx * dx * dx);
}
